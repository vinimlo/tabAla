...
import { validateCollectionName } from '@/lib/storage';
...
describe('validateCollectionName', () => {
  describe('empty name validation', () => {
    it('should reject empty string', () => {
      ...
      expect(result.error).toBe('Nome da coleção não pode estar vazio');
    });
  ...
  describe('duplicate name validation', () => {
    it('should reject exact duplicate name', () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.error).toBe('Já existe uma coleção com este nome');
    });
